...
class SmartFilter extends CBitrixComponent
{
	...
	public function executeComponent()
	{
		...
		return ['filter' => $this->getFilterForElements(), 'filterParams' => self::apiParams()];
	}
	...
	{	
		...
		{
			if(array_key_exists('MAX', $item->getValues()))
				$params[$item->code] = ['name' => $item->name, 'code' => $item->code, 'type' => 'range'];
			else
				$params[$item->code] = ['name' => $item->name, 'code' => $item->getFilterViewCode(), 'type' => 'list'];

			$params[$item->code]['OPTIONS'] = $item->getValues();
		}
	...
	public function getFilterForElements()
	{
		...
		{
			$filterKey = ($property->isSkuProperty() || $property instanceof \PriceField) ? 'offers' : 'products';
